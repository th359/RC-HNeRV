...
class VideoDataSet_FFT(Dataset):
    def __init__(self, args):
        self.video = [os.path.join(args.data_path, x) for x in sorted(os.listdir(args.data_path))]
        # Resize the input video and center crop
        self.crop_list, self.resize_list = args.crop_list, args.resize_list
    ...
    def __getitem__(self, idx):
        img_base = self.img_load(idx)
        img = self.img_transform(img_base)

        size = (img.shape[1]//self.size_n, img.shape[2]//self.size_n)
        size2 = (img.shape[1], img.shape[2])
        resize_img = resize(img, size, interpolation=transforms.InterpolationMode.BICUBIC, antialias=True)
        lr_img = resize(resize_img, size2, interpolation=transforms.InterpolationMode.BICUBIC, antialias=True)
        
        tensor_image = img/255.
        tensor_resize= resize_img/255.
        tensor_lr = lr_img/255.

        norm_idx = float(idx) / len(self.video)
        sample = {'gt': tensor_image, 'lr': tensor_lr, 'small': tensor_resize, 'idx': idx, 'norm_idx': norm_idx}
...
def OutImg(x, out_bias='tanh', flag=False):
    if out_bias == 'sigmoid':
...
##############################FFT_HNeRV and Decoder################################
class FFT_HNeRV(nn.Module):
    def __init__(self, args):
        super().__init__()
    ...
    def forward(self, input, lr, input_embed=None, encode_only=False):
        if input_embed != None:
        ...
        residue = OutImg(self.head_layer(output), self.out_bias, flag=True)
        ...
        return  img_out, residue, embed_list, dec_time
...
class HNeRVDecoder(nn.Module):
    ...
    def forward(self, img_embed):
        ...
        return  OutImg(output, self.out_bias, flag=True)
