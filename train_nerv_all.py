...
from model_all import VideoDataSet, VideoDataSet_FFT, HNeRV, HNeRVDecoder, FFT_HNeRV, TransformInput
...
def main():
    ...
    parser.add_argument('--use_rchnerv', action='store_true', default=False, help='proposed method(True) or HNeRV(False)')
    parser.add_argument('--size_n', type=int, default=128)
    #evaluation
...
def train(local_rank, args):
    ...
    # setup dataloader    
    if args.use_rchnerv:
        full_dataset = VideoDataSet_FFT(args)
    else:
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        for i, sample in enumerate(train_dataloader):
            ...
            if args.use_rchnerv:
                img_gt = data_to_gpu(sample['gt'], device)
                lr_img = data_to_gpu(sample['lr'], device)
                # small_img = data_to_gpu(sample['small'], device)
                ...
                img_out, residue, _, _ = model(cur_input, lr_img)
                ...
                if 5 == img_idx and (epoch % 15 == 0 or epoch == args.epochs-1) :
                    residue_gt = img_gt - lr_img
                    diff = residue_gt - residue
                    l = [diff, img_gt, img_out, residue_gt, residue, lr_img]
                ...
                if i % args.print_freq == 0 or i == len(train_dataloader) - 1:
                    ...
                    print_str = '[{}] Rank:{}, Epoch[{}/{}], Step [{}/{}], lr:{:.2e} pred_PSNR: {}, final_loss: {}'.format(
                        datetime.now().strftime("%Y/%m/%d %H:%M:%S"), local_rank, epoch+1, args.epochs, i+1, len(train_dataloader), lr, 
                        RoundTensor(pred_psnr, 2), RoundTensor(final_loss, 4))
                    print(print_str, flush=True)
                    if local_rank in [0, None]:
                        with open('{}/rank0.txt'.format(args.outf), 'a') as f:
                            f.write(print_str + '\n')
            # -----use VideoDataSet---------------------
                ...
                if 5 == img_idx and (epoch % 15 == 0 or epoch == args.epochs-1) :
                    residue = img_gt - img_out
                    l = [img_gt, img_out, residue]
                    for idx in range(len(l)):
                        d = l[idx].squeeze(0)
                        d = d.to('cpu').detach().numpy().copy()
                        if idx == 0 or idx == 1:
    ...
    for model_ind, cur_model in enumerate(model_list):
        ...
        for i, sample in enumerate(full_dataloader):
            if args.use_rchnerv:
                ...
                img_gt = data_to_gpu(sample['gt'], device)
                lr_img = data_to_gpu(sample['lr'], device)
                # small = data_to_gpu(sample['small'], device)
                # norm_idx = data_to_gpu(sample['norm_idx'], device)
                img_idx = data_to_gpu(sample['idx'], device)

                img_data = img_gt
                cur_input = img_gt
                img_out, residue, embed_list, dec_time = cur_model(cur_input, lr_img, dequant_vid_embed[i] if model_ind else None)
                ...
                if args.eval_fps:
                    time_list.pop()
                    for _ in range(100):
                        img_out, residue, embed_list, dec_time = cur_model(cur_input, embed_list[0])
                        time_list.append(dec_time)
